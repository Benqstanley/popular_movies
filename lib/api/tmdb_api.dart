...
class TMDBAPI {
  ...
  Future<List<MovieOverview>> fetchPopularMovies(int pageNumber) async {
    ...
    final response = await get(
        "http://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc&page=$pageNumber&api_key=$_apiKey");
    if (response.statusCode == 200) {
      Map<String, dynamic> jsonMap = json.decode(response.body);
      if (jsonMap.containsKey("results")) {
        return (jsonMap["results"] as List<dynamic>).map<MovieOverview>((movieMap) {
  ...
  Future<Map<String, dynamic>> search(String searchTerm) async {
    final response = await get(
        "https://api.themoviedb.org/3/search/movie?api_key=$_apiKey&language=en-US&query=${_prepareSearchTerm(searchTerm)}&page=1&include_adult=false");
    if (response.statusCode == 200) {
      return json.decode(response.body);
    }
    return null;
